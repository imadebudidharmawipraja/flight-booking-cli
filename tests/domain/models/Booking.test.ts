...
describe('Booking Model', () => {
  describe('constructor', () => {
    it('should create a booking with all properties', () => {
      const booking = new Booking('001', 'Budi', 'R001', 'BOOKED', 5, 1);
  ...
  describe('booking status', () => {
    it('should handle BOOKED status', () => {
      const booking = new Booking('004', 'Adi', 'R005', 'BOOKED', 2, 2);
      expect(booking.status).toBe('BOOKED');
    });

    it('should handle DEPARTED status', () => {
      const booking = new Booking('005', 'Agus', 'R006', 'DEPARTED', 4, 3);
      expect(booking.status).toBe('DEPARTED');
    });

    it('should handle ARRIVED status', () => {
      const booking = new Booking('006', 'Fajar', 'R007', 'ARRIVED', 6, 4);
