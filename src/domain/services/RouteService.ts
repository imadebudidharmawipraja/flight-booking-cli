...
export class RouteService implements IRouteService {
  ...
  private getAvailableSeat(routeId: string, flightId: string): number {
    ...
    const flightCapacity = this.flightRepository.findById(flightId)?.capacity || 0;
  ...
  private findTransitRoutes(
    ...
    const routes = this.repository.findAll().filter((route: Route) => route.scheduledDay > currentDay);

    routes.find((firstFlight) => {
      const firstFlightAvailableSeat = this.getAvailableSeat(firstFlight.id, firstFlight.flightId);

      if (firstFlight.departureCity === initialDeparture && firstFlightAvailableSeat > 0) {
        routes.find((secondFlight) => {
          const secondFlightAvailableSeat = this.getAvailableSeat(secondFlight.id, secondFlight.flightId)
          ...
          if (
            ...
            message.push(`flight from ${firstFlight.departureCity} - ${firstFlight.destinationCity} - ${secondFlight.destinationCity}  on day ${firstFlight.scheduledDay}`)
            message.push(`first flight available seat: ${firstFlightAvailableSeat}`)
            message.push(`second flight available seat: ${secondFlightAvailableSeat}`)
  ...
  public getAvailableBookFlight(
    ...
    const directFlight = availableByDay.find((route: Route) => {
      ...
      if (availableSeat > 0) {
        message.push(`flight found: ${departure} - ${destination} on day ${route.scheduledDay}`);
